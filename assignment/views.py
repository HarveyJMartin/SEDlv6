...
def all_assignments(request):
    ...
    return render(request, 'assignment/all_assignments.html', {'assignments': assignments})
...
def edit_assignment(request, assignment_id):
    ...
    return render(request, 'assignment/edit_assignment.html', {'form': form, 'assignment': assignment})
...
def user_assignments(request):
    ...
    assignments = Assignment.objects.filter(user=request.user).order_by('-assigned_date')
    return render(request, 'assignment/user_assignments.html', {'assignments': assignments})
...
def propose_edit(request, assignment_id):
    ...
    if assignment.pending_changes or assignment.status == 'deletion_requested':
        messages.error(request, "This assignment already has a pending request. Please wait for it to be resolved.")
...
def request_deletion(request, assignment_id):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, "Your deletion request has been submitted successfully.")
        return redirect('assignments')  # Redirect to My Assignments page
    return render(request, 'assignment/request_deletion.html', {'assignment': assignment})
...
def approve_changes(request, assignment_id):
    ...
    return render(request, 'assignment/approve_changes.html', {
...
def reject_changes(request, assignment_id):
    ...
    return render(request, 'assignment/reject_changes.html', {
        'assignment': assignment,
        'changes': changes,
    })
